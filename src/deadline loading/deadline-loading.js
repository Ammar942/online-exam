...
$(document).ready(function () {
  ...
  var deadlineAnimation = function () {
    setTimeout(function () {
      $("#designer-arm-grop").css({ "animation-duration": "2.25s" });
    }, 0);
    ...
    setTimeout(function () {
      $("#designer-arm-grop").css({ "animation-duration": "1.05s" });
    }, 12000);

    setTimeout(function () {
      $("#designer-arm-grop").css({ "animation-duration": "0.45s" });
    }, 18000);
  ...
  function timer(totalTime, deadline) {
    ...
    function countTime() {
      --actualDay;
      $(".deadline-days .day").text(`Timer ${actualDay}`);
      if (actualDay == 0) {
        clearInterval(timer);
